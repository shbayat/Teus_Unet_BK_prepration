...
## saves High inv data for resized images
p1_90  256x256
...
#        tk.Tk.__init__(self)
        ...
        self.ValP=[]
        self.TestP=[]
        self.balance=1 # used to be 0, if 1=balanced
...
#        colors = plt.cm.BuPu(np.linspace(0.5, 1, len(patients)+1))
    ...
    def PartitionWin(self):
        ...
        self.DS_resflag = IntVar()
        Checkbutton(self.frame1, text="DS_FFT_res", variable=self.DS_resflag).grid(row=9, column = 1)
    ...
    def Save(self):
        ...
        self.DS_res=self.DS_resflag.get()
...
#        inside_dir_DS='/BMode/ROI_Data/Down_Sample/' #Downsample/
        inside_dir_DS='/BMode/ROI_Data/Cut_axial/' #Zero padded /
        Benign_Cids=[]
...
#            file_pre='masked_FFT_res_' ## for resized data
            file_pre='Masked_FFT_'  ## for zero padded data
            inside_dir=inside_dir_DS
...
#        Test_BPids=[92,93,95,96,98,99,101,103,104,105,108,109,110]
  ################# #################################################
       ## saving High inv data
       # part 1 train: p2-30
#        Train_BPids=[4,2,10,25,29,21,27] #Part 1 train
#        Train_BPids=[50,39,40] #Part 2 train 31-60
#        Train_BPids=[68,69,72,74,76,77,70] #Part 3 train 61-90
#       
        Val_BPids=[]
        Train_BPids=[]
#        Train_BPids=[99,100]
        ...
        ########################## saving val high inv data
#        Val_BPids=[14,24,30,37] # part 1 : 2-45
#        Val_BPids=[46,61] # part 2 : 46-90
#        Test_BPids=[92,93,95,96,98,99,101,103,104,105,108,109,110]
#        Test_BPids=[92,93,95,96,98,99] #part 1 test
#        Test_BPids=[101,103,104,105,108,109,110]#part 2 test= 101-110
        
        ###### version 2 of test to have all inv
        
#        Test_BPids=[92,93,94,95,96,98,99] #part 1 test
        Test_BPids=[101,102,103,104,105,106,107,108,109,110]#part 2 test= 101-110
#        ############for p111-125
#        Test_BPids=[112,114,117,120,121,122,124,125]  #part1
#        Test_BPids=[128,130,131,132,134,135,136]  #part2
        #Select Benign Cores
...
#        val_set.append(saveData(Val_BCids,self.RF))
        ...
        train_set=[]
        train_set.append(saveData(Train_CCids,self.DS_res))
        train_set.append(saveData(Train_BCids,self.DS_res))

        #Test
        test_set=[]
        test_set.append(saveData(Test_CCids,self.DS_res))
        test_set.append(saveData(Test_BCids,self.DS_res))

        #Validation
        val_set=[]
        val_set.append(saveData(Val_CCids,self.DS_res))
        val_set.append(saveData(Val_BCids,self.DS_res))
...
##                    ## Train data-saving
#                    hdf5storage.savemat('Z:/shared/images/ProstateVGH-2/Data/Dataset/InProstate/Zero-pad/BK_DS_FFT_zp_train_bal_P%s_%s_%s_%s_%s.mat' %(self.minP.get(),self.maxP.get(),s1,s2,timestr),{
#                                        'data_train':data_train,'label_train':np.asarray(label_train,dtype=float),'inv_train':np.asarray(inv_train,dtype=float),'GS_train':GS_train,'PatientId_train':np.asarray(PatientId_train,dtype=int),'idcore_train':np.asarray(idcore_train,dtype=int),
#                                        'RF_freq':self.RF_freq})
#
#                    print('Train Data is saved at:Z:/shared/images/ProstateVGH-2/Data/Dataset/InProstate/Zero-pad/BK_DS_FFT_zp_train_P%s_%s_%s_%s_%s.mat' %(self.minP.get(),self.maxP.get(),s1,s2,timestr))

#                  ## Val data-saving
                    
#                    hdf5storage.savemat('Z:/shared/images/ProstateVGH-2/Data/Dataset/InProstate/Zero-pad/BK_DS_FFT_zp_val_bal_P%s_%s_%s_%s_%s.mat' %(self.minP.get(),self.maxP.get(),s1,s2,timestr),{
#                                        'data_val':data_val,'label_val':np.asarray(label_val,dtype=float),'inv_val':np.asarray(inv_val,dtype=float),'GS_val':GS_val,'PatientId_val':np.asarray(PatientId_val,dtype=int),'idcore_val':np.asarray(idcore_val,dtype=int),
#                                        'RF_freq':self.RF_freq})
#
#                    print('Val Data is saved at:Z:/shared/images/ProstateVGH-2/Data/Dataset/Zero-pad/BK_DS_FFT_zp_val_P%s_%s_%s_%s_%s.mat' %(self.minP.get(),self.maxP.get(),s1,s2,timestr))

                    ## Test data-saving
                    hdf5storage.savemat('Z:/shared/images/ProstateVGH-2/Data/Dataset/InProstate/Zero-pad/BK_DS_FFT_zp_test_P%s_%s_%s_%s_%s.mat' %(self.minP.get(),self.maxP.get(),s1,s2,timestr),{
                                       'data_test':data_test,'label_test':np.asarray(label_test,dtype=float),'inv_test':np.asarray(inv_test,dtype=float),'GS_test':GS_test,'PatientId_test':np.asarray(PatientId_test,dtype=int),'idcore_test':np.asarray(idcore_test,dtype=int),
                                        'RF_freq':self.RF_freq})

                    print('Test Data is saved at:Z:/shared/images/ProstateVGH-2/Data/Dataset/InProstate/Zero-pad/BK_DS_FFT_zp_test_P%s_%s_%s_%s_%s.mat' %(self.minP.get(),self.maxP.get(),s1,s2,timestr))
...
#   return CCids
def saveData(CCids,DS_res):
   
   for i in range(len(CCids)):
        data=spio.loadmat(CCids[i].fname)
#        
        if DS_res:
            CCids[i].data=data["masked_FFT"]
        else:
            CCids[i].data=data["meanRF"]   
        

#        CCids[i].data=data["masekd_FFT_res"]
