...
%     
    ...
    CoreNo=size(S_IQ,1);
    save_dir=[data_dir,'\BMode\ROI_Data\Cut_axial\'];
...
%         figure(8);imagesc(log10(1+abs(squeeze(masked_RF(:,:,1)))));colormap('gray');
        ...
        %%% saving DS RF
        RF_DSfile=[save_dir,strrep(S_IQ(CoreID).name,'IQ','RF_cut_a')];
        RF_DSfile2=strrep(RF_DSfile,'.dat','.mat');
        save(RF_DSfile2,'RF_down');
        
        %%% saving cut mask
        masked_DSfile=[save_dir,strrep(S_IQ(CoreID).name,'IQ','masked_cut_a')];
        masked_DSfile2=strrep(masked_DSfile,'.dat','.mat');
        save(masked_DSfile2,'masked_RF');
        masked_DSfile4=[save_dir,strrep(S_IQ(CoreID).name,'IQ','masked1_cut_a')];
        masked_DSfile3=strrep(masked_DSfile4,'.dat','.jpg');
        imwrite(mask_down,masked_DSfile3,'jpg');
        masked_DSfile5=strrep(masked_DSfile4,'.dat','.mat');
        mask_RF_cut=mask_down;
        save(masked_DSfile5,'mask_RF_cut');
    end
end
