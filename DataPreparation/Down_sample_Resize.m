...
%         figure(1);imagesc(log10(1+abs(squeeze(rf_AllFrames(:,:,1)))));colormap('gray');
        rf_AllFrames_c_top=rf_AllFrames(1:1536,:,:);
...
%         end
        ...
        rf_AllFrames_c=imresize(rf_AllFrames_c_top,[256,256]);

%         figure(2);imagesc(log10(1+abs(squeeze(rf_AllFrames_c_top(:,:,1)))));colormap('gray');
%         figure(3);imagesc(log10(1+abs(squeeze(rf_AllFrames_c(:,:,1)))));colormap('gray');
        RF_down = rf_AllFrames_c;
        
        %%% read mask
        mask_file= strcat(data_dir3,S_mask(CoreID).name);
        mask=imread(mask_file);
%         figure(5);imagesc(mask(:,:));colormap('gray');
        mask_c_top=mask(1:1536,:);
        mask_down= imresize(mask_c_top,[256,256]);
%         figure(6);imagesc(mask_down);colormap('gray');
...
%         figure(8);imagesc(log10(1+abs(squeeze(masked_RF(:,:,1)))));colormap('gray');
        ...
        %%% saving DS RF
        RF_DSfile=[save_dir,strrep(S_IQ(CoreID).name,'IQ','RF_DS_res')];
        RF_DSfile2=strrep(RF_DSfile,'.dat','.mat');
        save(RF_DSfile2,'RF_down');
        
        %%% saving DS mask
        masked_DSfile=[save_dir,strrep(S_IQ(CoreID).name,'IQ','masked_DS_res')];
