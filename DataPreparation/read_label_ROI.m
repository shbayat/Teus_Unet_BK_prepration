function read_label_ROI(Patient)
    ...
    save_dir=[data_dir,'\BMode\ROI_Data\Down_Sample\Labels\'];
...
%         figure(2);imagesc(ROI_mask_c);
        ROI_mask_down = ROI_mask_c(1:d_rate:end,1:DS_L:end,:);
...
% %%%   Inverse to cancer
        if label==0
            ROI_mask_down_2 =2*ones(size(ROI_mask_down,1),size(ROI_mask_down,2));
            for i=1:size(ROI_mask_down,1)
                for j=1:size(ROI_mask_down,2)
                    if ROI_mask_down(i,j)==0
                        ROI_mask_down_2(i,j)=255;
                    elseif ROI_mask_down(i,j)==255
                        ROI_mask_down_2(i,j)=0;
                    end
                end
            end
           ROI_mask_down=ROI_mask_down_2; 
        ...
        if flag==0
            ...
            ROI_mask_DS_file_2=[save_dir,strrep(RF_ROI(CoreID).name,'.bmp','_2.mat')];
        else
            ROI_mask_DS_file=[save_dir,strrep(RF_ROI(CoreID).name,'.jpg','.mat')];
            ROI_mask_DS_file_2=[save_dir,strrep(RF_ROI(CoreID).name,'.jpg','_2.mat')];
        end
...
%%% for new savings
        save(ROI_mask_DS_file_2,'ROI_mask_down');
        ROI_mask_DS_file_2=strrep(ROI_mask_DS_file_2,'.mat','.bmp');
        imwrite(ROI_mask_down,ROI_mask_DS_file_2,'bmp');
