...
#    DICOM_Path=folderp+subfolders[0]+'\\Images'
    folders = []
    ...
    cur.execute(query_str)
    query_str = "UPDATE patient SET dt = " +str(dT)+ " WHERE ID = " + str(Patient)
    cur.execute(query_str)
    query_str = "UPDATE patient SET dR = " + str(dR) + " WHERE ID = "+ str(Patient)
    cur.execute(query_str)
    query_str = "UPDATE patient SET RFwidth = " + str(Width) + " WHERE ID = "+ str(Patient)
    cur.execute(query_str)
    query_str = "UPDATE patient SET RFhight = " + str(Height) + " WHERE ID = "+ str(Patient)
    cur.execute(query_str)
    cur.close()
    cnx.commit()
